...
import { CHAT_AUTO_MESSAGE_CONFIG_KEY } from "@/backend/lib-server/chat-service";
...
export async function GET(request: Request) {
  ...
  return jsonWithSecurity({
    ...
    notesByApplicantId,
    chatAutoMessage:
      typeof dashboardConfig[CHAT_AUTO_MESSAGE_CONFIG_KEY] === "string"
        ? dashboardConfig[CHAT_AUTO_MESSAGE_CONFIG_KEY]
        : "",
    dashboardConfig,
    billingHistory,
  });
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const nextChatAutoMessage =
      typeof body.chatAutoMessage === "string"
        ? sanitizePlainTextInput(body.chatAutoMessage, 1200)
        : null;

    const moderatedNotesByApplicantId = await censorProfanityInPayload(nextNotesByApplicantId);
    const moderatedChatAutoMessage = await censorProfanityInPayload(nextChatAutoMessage);
    ...
    const payload = {
      ...
      notesByApplicantId: moderatedNotesByApplicantId ?? currentNotesByApplicantId,
      dashboardConfig: {
        ...(nextDashboardConfig ?? currentDashboardConfig),
        [CHAT_AUTO_MESSAGE_CONFIG_KEY]:
          moderatedChatAutoMessage ??
          (typeof currentDashboardConfig[CHAT_AUTO_MESSAGE_CONFIG_KEY] === "string"
            ? currentDashboardConfig[CHAT_AUTO_MESSAGE_CONFIG_KEY]
            : ""),
      },
      billingHistory,
    };
    ...
    return jsonWithSecurity({
      ...
      chatAutoMessage:
        typeof payload.dashboardConfig[CHAT_AUTO_MESSAGE_CONFIG_KEY] === "string"
          ? payload.dashboardConfig[CHAT_AUTO_MESSAGE_CONFIG_KEY]
          : "",
    });
  } catch {
